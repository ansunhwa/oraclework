...
--4 NULL값이 들어가지 않도록
ALTER TABLE TB_CLASS_TYPE MODIFY CLASS_TYPE_NAME NOT NULL;
...
--7 PRIMARY KEY
ALTER TABLE TB_CLASS_TYPE 
RENAME CONSTRAINT SYS_C007402 TO PK_CALSS_TYPE_NO;
 -- PRIMARY KEY 원래 이름을 알고 있어야 바꿀 수 있음
...
AS SELECT STUDENT_NAME, DEPARTMENT_NAME, NVL(PROFESSOR_NAME,'지도교수없음')지도교수
   FROM TB_STUDENT
   JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
...
INSERT INTO TB_국어국문학과(STUDENT_NO, STUDENT_NAME,STUDENT_SSN, PROFESSOR_NAME)
                VALUES(STUDENT_NO, STUDENT_NAME, SUBSTR(STUDENT_SNN,1,2), PROFESSOR_NAME)
                 FROM TB_STUDENT
                JOIN TB_PROFESSOR ON (COACH_PROFESSOR_NO = PROFESSOR_NO)
                WHERE ;
