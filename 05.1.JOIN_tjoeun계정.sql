/*
    <JOIN>
    두개 이상의 테이블에서 데이터를 조회하고자 할때 사용하는 구문
    조회결과는 하나의 결과물(result set)로 나옴
    
    -관계형데이터베이스는 최소한의 데이터로 각각 테이블에 담고있음
    (중복을 최소화하기 위해)
    => 관계형 데이터베이스에서 SQL문을 이용한 테이블간의 "관계"를 맺는 방법
    
    -JOIN은 크게 "오라클 전용구문" 과 "ANSI구문"(ANSI == 미국국립표준협회)
    
    [JOIN 용어정리]
            오라클 전용 구문         |             ANSI
         -------------------------------------------------------
             등가 조인              |   내부조인(INNER JON) => JOIN USING/ON
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM  EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID;   --코드와 아이디가 같으면 / NULL은 제외
--일치하는 행만 가져오세요 / 일치하는 값이 없는 행은 조회 제외
...
--해결방법1) 테이블 명을 이용하는 방법(테이블.컬럼)
SELECT EMP_ID, EMP_NAME, EMPLOYEE.JOB_CODE, JOB_NAME
FROM EMPLOYEE, JOB
WHERE EMPLOYEE.JOB_CODE = JOB.JOB_CODE;

--해결방법2)테이블에 별칭을 부여하여 이용하는 방법(별칭부여.컬럼)
...
--해결방법2) JOIN USING구문 사용하는 방법(두 컬럼이 일치할 때만 사용가능)--별칭 안해도 됨
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
...
-- [참고]
-- NATURAL JOIN : 공통된 컬럼을 자동으로 매칭시켜줌  (컬럼이름이 다를 경우x)
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
NATURAL JOIN JOB;
...
--2. DEPARTMENT 과 LOCATION 참고 전체 부서의 부서코드, 부서명, 지역코드, 지역명 조회
--오라클
...
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 연봉
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); -- 기준이 되는 테이블의 반대편 테이블 컬럼에 컬럼 뒤에(+)를 붙혀줌

-- 2) RIGHT [OUTER] JOIN : 두 테이블 중 오른쪽에 기술된 테이블을 기준으로 JOIN
-- >>ANSI구문
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 연봉
FROM EMPLOYEE
RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- DEPARTMENT 기준으로
...
-- 3) FULL [OUTER] JOIN : 두 테이블이 가진 모든 행 조회(오라클전용은 사용x)
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12 연봉
FROM EMPLOYEE
...
--                       사수사번, 사수명, 사수의 부서코드EMPLOYEE M 조회
-- >> 오라클 전용 구문
--1) 사수가 있는 사원만 조회
SELECT E.EMP_ID, E.EMP_NAME, E.DEPT_CODE, M.EMP_ID, M.EMP_NAME, M.DEPT_CODE
FROM EMPLOYEE E, EMPLOYEE M
WHERE E.MANAGER_ID = M.EMP_ID;  -- ID가 서로 같은거 여서 NULL값 제외 
...
SELECT E.EMP_ID, E.EMP_NAME, E.DEPT_CODE, M.EMP_ID, M.EMP_NAME, M.DEPT_CODE
FROM EMPLOYEE E
LEFT JOIN EMPLOYEE M ON (E.MANAGER_ID = M.EMP_ID);   --LEFT빼면 NULL 미포함
...
WHERE DEPT_CODE = DEPT_ID AND E.JOB_CODE = J.JOB_CODE;  --컬럼이 동일하면 테이블명(별칭)필요

-- >> ANSI구문
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE E
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE);   -- 컬럼 이름이 같으므로 USING사용
...
-- 2. 사번, 사원명, 부서명, 직급명, 지역명, 국가명, 급여등급 조회(모든테이블 
--오라클
