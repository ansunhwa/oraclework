...
-- 2. 접속을 위한 CREATE SESSION 권한 보여
GRANT CREATE SESSION TO SAMPLE;
  --접속하여 테이블을 만들면 오류. 테이블을 만들 수 있는 권한없음
  
-- 3. 테이블을 생성할 수 있는 권한 부여
GRANT CREATE TABLE TO SAMPLE;

-- 4. 테이블에 데이터를 삽입하려면 tablespace를 줘야한다
ALTER USER SAMPLE default tablespace users quota unlimited on users;  --샘플에게 테이블스페이스를 제한없게

--------------------------------------------------------------
/*
    객체 접근 권한 종류
    특정 개체에 접근하여 조작할 수 있는 권한
...
-- 5. SAMPLE계정에게 TJOEUN계정에 EMPLOYEE테이블을 SELECT할 수 있는 권한
GRANT SELECT ON TJOEUN.EMPLOYEE TO SAMPLE;

-- 6. SAMPLE계정에게 TJOEUN계정에 EMPLOYEE테이블을 INSERT할 수 있는 권한
...
/*
    ...
    CONNECT : CREATE, SESSION    두개 다 한번에
    RESOURCE : CREATE TABLE, CREATE SEQUENCE, ....
    DBA : 시스템 및 객체관리에 대한 모든 권한을 가지고 있는 롤
*/

GRANT CONNECT, RESOURCE TO SAMPLE;  -- 할 수있는거 거의 다 사용할 수 있다
